...
class AdminAdminDebatesViewPageActions extends PageActions {
  NavigationState navigation;
  final messageHandler = locator<MessageHandler>();

  final AdminDebateStore targetStore;
  final AdminAdminDebatesViewPageStore pageStore;
  final AdminAdminDebatesViewConfig pageConfig;

  AdminAdminDebatesViewPageActions(this.navigation, this.targetStore, this.pageStore, this.pageConfig);
  ...
  List<Widget> getActions(BuildContext context) {
    ...
    list.add(JudoAppBarButton(
      ...
      onPressed: () async {
        try {
          await pageStore.deleteDebate(targetStore);
    ...
    list.add(JudoAppBarButton(
      ...
      onPressed: () async {
        ...
        AdminDebateStore cloned = AdminDebateStore.clone(targetStore);
        var res = await navigation.open(Routes.adminAdminDebatesUpdatePage, arguments: AdminAdminDebatesUpdatePageArguments(targetStore: cloned));
        if (res != null) {
          try {
            await pageStore.updateDebate(cloned, targetStore);
          } catch (error) {
