...
class AdminConCreatedByViewPageStore extends _AdminConCreatedByViewPageStore with _$AdminConCreatedByViewPageStore {}

abstract class _AdminConCreatedByViewPageStore with Store {
  ...
  _AdminConCreatedByViewPageStore() {
  ...
  void initSortAggregatedLists(BuildContext context, AdminConCreatedByViewConfig pageConfig) {
    if (targetStore.activityCities != null) {
      var dataInfo = EdemokraciaAdminConCreatedByViewActivityCitiesMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.activityDistricts != null) {
      var dataInfo = EdemokraciaAdminConCreatedByViewActivityDistrictsMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.activityCounties != null) {
      var dataInfo = EdemokraciaAdminConCreatedByViewActivityCountiesMobileTable(context, this, pageConfig, disabled: false);
  ...
  void activityCitiesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConCreatedByViewActivityCities',
  ...
  void activityCountiesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConCreatedByViewActivityCounties',
  ...
  void activityDistrictsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConCreatedByViewActivityDistricts',
...
class AdminConCreatedByViewPage extends StatefulWidget {
  static const String title = 'View / Edit User';

  final AdminUserStore targetStore;
  final AdminConStore ownerStore;

  const AdminConCreatedByViewPage({Key key, @required this.targetStore, @required this.ownerStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminConCreatedByViewPageState(targetStore: targetStore, ownerStore: ownerStore);
}

class _AdminConCreatedByViewPageState extends State<AdminConCreatedByViewPage> {
  AdminUserStore targetStore;
  final AdminConStore ownerStore;

  final tableService = locator<TableService>();
  final AdminConCreatedByViewPageStore pageStore = AdminConCreatedByViewPageStore();
  final AdminConCreatedByViewConfig pageConfig = AdminConCreatedByViewConfig();
  AdminConCreatedByViewPageActions pageActions;

  _AdminConCreatedByViewPageState({@required this.targetStore, @required this.ownerStore}) {
    ...
    pageActions = AdminConCreatedByViewPageActions(navigation, targetStore, ownerStore, pageStore, pageConfig);
    pageStore.refreshUser(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminConCreatedByViewPage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminConCreatedByViewMobilePage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminConCreatedByViewTabletPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminConCreatedByViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      return getAdminConCreatedByViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
    });
