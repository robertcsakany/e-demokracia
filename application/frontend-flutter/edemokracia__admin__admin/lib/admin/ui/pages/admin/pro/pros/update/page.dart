...
class AdminProProsUpdatePageStore extends _AdminProProsUpdatePageStore with _$AdminProProsUpdatePageStore {}

abstract class _AdminProProsUpdatePageStore with Store {
  ...
  void initSortAggregatedLists(BuildContext context, AdminProProsUpdateConfig pageConfig) {
    if (targetStore.cons != null) {
      var dataInfo = EdemokraciaAdminProProsEditConsMobileTable(context, this, pageConfig, disabled: true);
      targetStore.cons.sort(TableUtility.getSortComparator(consSortColumnIndex, consSortAsc, dataInfo));
    }
    if (targetStore.pros != null) {
      var dataInfo = EdemokraciaAdminProProsEditProsMobileTable(context, this, pageConfig, disabled: true);
    ...
    if (targetStore.comments != null) {
      var dataInfo = EdemokraciaAdminProProsEditCommentsMobileTable(context, this, pageConfig, disabled: true);
  ...
  void commentsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminProStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminProProsEditComments',
  ...
  void consSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminProStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminProProsEditCons',
  ...
  void prosSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminProStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminProProsEditPros',
...
class AdminProProsUpdatePage extends StatefulWidget {
  static const String title = 'View / Edit Pro';

  final AdminProStore targetStore;

  const AdminProProsUpdatePage({Key key, @required this.targetStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminProProsUpdatePageState(targetStore: targetStore);
}

class _AdminProProsUpdatePageState extends State<AdminProProsUpdatePage> {
  final AdminProStore targetStore;

  final tableService = locator<TableService>();
  final AdminProProsUpdatePageStore pageStore = AdminProProsUpdatePageStore();
  final AdminProProsUpdateConfig pageConfig = AdminProProsUpdateConfig();
  AdminProProsUpdatePageActions pageActions;

  _AdminProProsUpdatePageState({@required this.targetStore}) {
    ...
    pageActions = AdminProProsUpdatePageActions(navigation, targetStore, pageStore, pageConfig);
    pageStore.refreshPro(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminProProsUpdatePage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminProProsUpdateMobilePage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminProProsUpdateTabletPage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminProProsUpdateDesktopPage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      return getAdminProProsUpdateDesktopPage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
    });
