...
class AdminAdminCountiesTablePageActions extends PageActions {
  ...
  AdminAdminCountiesTablePageActions(this.navigation, this.pageStore, this.pageConfig);
  ...
  List<Widget> getActions(BuildContext context) {
    ...
    list.add(JudoAppBarButton(
      ...
      padding: EdgeInsets.symmetric(horizontal: 8.0),
      onPressed: () async {
        var created = await navigation.open(Routes.adminAdminCountiesCreatePage);
        if (created != null) {
          try {
            await pageStore.getCounties();
          } catch (error) {
