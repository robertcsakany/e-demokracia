...
class AdminDebateConsViewPageStore extends _AdminDebateConsViewPageStore with _$AdminDebateConsViewPageStore {}

abstract class _AdminDebateConsViewPageStore with Store {
  ...
  void initSortAggregatedLists(BuildContext context, AdminDebateConsViewConfig pageConfig) {
    if (targetStore.cons != null) {
      var dataInfo = EdemokraciaAdminDebateConsViewConsMobileTable(context, this, pageConfig, disabled: false);
      targetStore.cons.sort(TableUtility.getSortComparator(consSortColumnIndex, consSortAsc, dataInfo));
    }
    if (targetStore.pros != null) {
      var dataInfo = EdemokraciaAdminDebateConsViewProsMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.comments != null) {
      var dataInfo = EdemokraciaAdminDebateConsViewCommentsMobileTable(context, this, pageConfig, disabled: false);
  ...
  void commentsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminConStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminDebateConsViewComments',
  ...
  void consSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminConStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminDebateConsViewCons',
  ...
  void prosSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminConStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminDebateConsViewPros',
...
class AdminDebateConsViewPage extends StatefulWidget {
  static const String title = 'View / Edit Con';

  final AdminConStore targetStore;
  final AdminDebateStore ownerStore;

  const AdminDebateConsViewPage({Key key, @required this.targetStore, @required this.ownerStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminDebateConsViewPageState(targetStore: targetStore, ownerStore: ownerStore);
}

class _AdminDebateConsViewPageState extends State<AdminDebateConsViewPage> {
  AdminConStore targetStore;
  final AdminDebateStore ownerStore;

  final tableService = locator<TableService>();
  final AdminDebateConsViewPageStore pageStore = AdminDebateConsViewPageStore();
  final AdminDebateConsViewConfig pageConfig = AdminDebateConsViewConfig();
  AdminDebateConsViewPageActions pageActions;

  _AdminDebateConsViewPageState({@required this.targetStore, @required this.ownerStore}) {
    ...
    pageActions = AdminDebateConsViewPageActions(navigation, targetStore, ownerStore, pageStore, pageConfig);
    pageStore.refreshCon(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminDebateConsViewPage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminDebateConsViewMobilePage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminDebateConsViewTabletPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminDebateConsViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      return getAdminDebateConsViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
    });
