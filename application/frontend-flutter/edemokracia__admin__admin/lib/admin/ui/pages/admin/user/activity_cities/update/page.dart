...
class AdminUserActivityCitiesUpdatePageStore extends _AdminUserActivityCitiesUpdatePageStore with _$AdminUserActivityCitiesUpdatePageStore {}

abstract class _AdminUserActivityCitiesUpdatePageStore with Store {
  ...
  _AdminUserActivityCitiesUpdatePageStore() {
  ...
  void initSortAggregatedLists(BuildContext context, AdminUserActivityCitiesUpdateConfig pageConfig) {
    if (targetStore.districts != null) {
      var dataInfo = EdemokraciaAdminUserActivityCitiesEditDistrictsMobileTable(context, this, pageConfig, disabled: true);
  ...
  void districtsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminCityStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminUserActivityCitiesEditDistricts',
...
class AdminUserActivityCitiesUpdatePage extends StatefulWidget {
  static const String title = 'City';

  final AdminCityStore targetStore;

  const AdminUserActivityCitiesUpdatePage({Key key, @required this.targetStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminUserActivityCitiesUpdatePageState(targetStore: targetStore);
}

class _AdminUserActivityCitiesUpdatePageState extends State<AdminUserActivityCitiesUpdatePage> {
  final AdminCityStore targetStore;

  final tableService = locator<TableService>();
  final AdminUserActivityCitiesUpdatePageStore pageStore = AdminUserActivityCitiesUpdatePageStore();
  final AdminUserActivityCitiesUpdateConfig pageConfig = AdminUserActivityCitiesUpdateConfig();
  AdminUserActivityCitiesUpdatePageActions pageActions;

  _AdminUserActivityCitiesUpdatePageState({@required this.targetStore}) {
    ...
    pageActions = AdminUserActivityCitiesUpdatePageActions(navigation, targetStore, pageStore, pageConfig);
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminUserActivityCitiesUpdateMobilePage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminUserActivityCitiesUpdateTabletPage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminUserActivityCitiesUpdateDesktopPage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      return getAdminUserActivityCitiesUpdateDesktopPage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
    });
