...
class AdminConProsViewPageStore extends _AdminConProsViewPageStore with _$AdminConProsViewPageStore {}

abstract class _AdminConProsViewPageStore with Store {
  ...
  void initSortAggregatedLists(BuildContext context, AdminConProsViewConfig pageConfig) {
    if (targetStore.cons != null) {
      var dataInfo = EdemokraciaAdminConProsViewConsMobileTable(context, this, pageConfig, disabled: false);
      targetStore.cons.sort(TableUtility.getSortComparator(consSortColumnIndex, consSortAsc, dataInfo));
    }
    if (targetStore.pros != null) {
      var dataInfo = EdemokraciaAdminConProsViewProsMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.comments != null) {
      var dataInfo = EdemokraciaAdminConProsViewCommentsMobileTable(context, this, pageConfig, disabled: false);
  ...
  void commentsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminProStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConProsViewComments',
  ...
  void consSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminProStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConProsViewCons',
  ...
  void prosSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminProStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConProsViewPros',
...
class AdminConProsViewPage extends StatefulWidget {
  static const String title = 'View / Edit Pro';

  final AdminProStore targetStore;
  final AdminConStore ownerStore;

  const AdminConProsViewPage({Key key, @required this.targetStore, @required this.ownerStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminConProsViewPageState(targetStore: targetStore, ownerStore: ownerStore);
}

class _AdminConProsViewPageState extends State<AdminConProsViewPage> {
  AdminProStore targetStore;
  final AdminConStore ownerStore;

  final tableService = locator<TableService>();
  final AdminConProsViewPageStore pageStore = AdminConProsViewPageStore();
  final AdminConProsViewConfig pageConfig = AdminConProsViewConfig();
  AdminConProsViewPageActions pageActions;

  _AdminConProsViewPageState({@required this.targetStore, @required this.ownerStore}) {
    ...
    pageActions = AdminConProsViewPageActions(navigation, targetStore, ownerStore, pageStore, pageConfig);
    pageStore.refreshPro(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminConProsViewPage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminConProsViewMobilePage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminConProsViewTabletPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminConProsViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      return getAdminConProsViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
    });
