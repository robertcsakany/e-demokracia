...
class AdminIssueOwnerViewPageStore extends _AdminIssueOwnerViewPageStore with _$AdminIssueOwnerViewPageStore {}

abstract class _AdminIssueOwnerViewPageStore with Store {
  ...
  void initSortAggregatedLists(BuildContext context, AdminIssueOwnerViewConfig pageConfig) {
    if (targetStore.activityCities != null) {
      var dataInfo = EdemokraciaAdminIssueOwnerViewActivityCitiesMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.activityDistricts != null) {
      var dataInfo = EdemokraciaAdminIssueOwnerViewActivityDistrictsMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.activityCounties != null) {
      var dataInfo = EdemokraciaAdminIssueOwnerViewActivityCountiesMobileTable(context, this, pageConfig, disabled: false);
  ...
  void activityCitiesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminIssueOwnerViewActivityCities',
  ...
  void activityCountiesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminIssueOwnerViewActivityCounties',
  ...
  void activityDistrictsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminIssueOwnerViewActivityDistricts',
...
class AdminIssueOwnerViewPage extends StatefulWidget {
  static const String title = 'View / Edit User';

  final AdminUserStore targetStore;
  final AdminIssueStore ownerStore;

  const AdminIssueOwnerViewPage({Key key, @required this.targetStore, @required this.ownerStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminIssueOwnerViewPageState(targetStore: targetStore, ownerStore: ownerStore);
}

class _AdminIssueOwnerViewPageState extends State<AdminIssueOwnerViewPage> {
  AdminUserStore targetStore;
  final AdminIssueStore ownerStore;

  final tableService = locator<TableService>();
  final AdminIssueOwnerViewPageStore pageStore = AdminIssueOwnerViewPageStore();
  final AdminIssueOwnerViewConfig pageConfig = AdminIssueOwnerViewConfig();
  AdminIssueOwnerViewPageActions pageActions;

  _AdminIssueOwnerViewPageState({@required this.targetStore, @required this.ownerStore}) {
    ...
    pageActions = AdminIssueOwnerViewPageActions(navigation, targetStore, ownerStore, pageStore, pageConfig);
    pageStore.refreshUser(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminIssueOwnerViewPage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminIssueOwnerViewMobilePage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminIssueOwnerViewTabletPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminIssueOwnerViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      return getAdminIssueOwnerViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
    });
