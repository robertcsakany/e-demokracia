...
class AdminCommentVotesTablePageActions extends PageActions {
  NavigationState navigation;
  final messageHandler = locator<MessageHandler>();

  final AdminCommentStore ownerStore;
  final AdminCommentVotesTablePageStore pageStore;
  final AdminCommentVotesTableConfig pageConfig;

  AdminCommentVotesTablePageActions(this.navigation, this.ownerStore, this.pageStore, this.pageConfig);
  ...
  List<Widget> getActions(BuildContext context) {
    ...
    list.add(JudoAppBarButton(
      ...
      padding: EdgeInsets.symmetric(horizontal: 8.0),
      onPressed: () async {
        var created = await navigation.open(Routes.adminCommentVotesCreatePage, arguments: AdminCommentVotesCreatePageArguments(ownerStore: ownerStore));
        if (created != null) {
          try {
