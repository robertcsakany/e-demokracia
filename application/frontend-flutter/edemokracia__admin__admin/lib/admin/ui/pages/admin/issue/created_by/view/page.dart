...
class AdminIssueCreatedByViewPageStore extends _AdminIssueCreatedByViewPageStore with _$AdminIssueCreatedByViewPageStore {}

abstract class _AdminIssueCreatedByViewPageStore with Store {
  ...
  _AdminIssueCreatedByViewPageStore() {
  ...
  void initSortAggregatedLists(BuildContext context, AdminIssueCreatedByViewConfig pageConfig) {
    if (targetStore.activityCities != null) {
      var dataInfo = EdemokraciaAdminIssueCreatedByViewActivityCitiesMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.activityDistricts != null) {
      var dataInfo = EdemokraciaAdminIssueCreatedByViewActivityDistrictsMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.activityCounties != null) {
      var dataInfo = EdemokraciaAdminIssueCreatedByViewActivityCountiesMobileTable(context, this, pageConfig, disabled: false);
  ...
  void activityCitiesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminIssueCreatedByViewActivityCities',
  ...
  void activityCountiesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminIssueCreatedByViewActivityCounties',
  ...
  void activityDistrictsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminIssueCreatedByViewActivityDistricts',
...
class AdminIssueCreatedByViewPage extends StatefulWidget {
  static const String title = 'View / Edit User';

  final AdminUserStore targetStore;
  final AdminIssueStore ownerStore;

  const AdminIssueCreatedByViewPage({Key key, @required this.targetStore, @required this.ownerStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminIssueCreatedByViewPageState(targetStore: targetStore, ownerStore: ownerStore);
}

class _AdminIssueCreatedByViewPageState extends State<AdminIssueCreatedByViewPage> {
  AdminUserStore targetStore;
  final AdminIssueStore ownerStore;

  final tableService = locator<TableService>();
  final AdminIssueCreatedByViewPageStore pageStore = AdminIssueCreatedByViewPageStore();
  final AdminIssueCreatedByViewConfig pageConfig = AdminIssueCreatedByViewConfig();
  AdminIssueCreatedByViewPageActions pageActions;

  _AdminIssueCreatedByViewPageState({@required this.targetStore, @required this.ownerStore}) {
    ...
    pageActions = AdminIssueCreatedByViewPageActions(navigation, targetStore, ownerStore, pageStore, pageConfig);
    pageStore.refreshUser(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminIssueCreatedByViewPage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminIssueCreatedByViewMobilePage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminIssueCreatedByViewTabletPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminIssueCreatedByViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      return getAdminIssueCreatedByViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
    });
