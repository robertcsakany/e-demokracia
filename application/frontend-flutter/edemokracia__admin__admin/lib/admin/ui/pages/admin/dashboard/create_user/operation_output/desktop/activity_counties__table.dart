...
class EdemokraciaAdminDashboardCreateUserOutputActivityCountiesDesktopTable implements JudoTableDataInfo {
  const EdemokraciaAdminDashboardCreateUserOutputActivityCountiesDesktopTable(this.context, this.pageStore, this.pageConfig, {this.disabled = false});

  final BuildContext context;
  final bool disabled;
  final AdminDashboardCreateUserOperationOutputPageStore pageStore;
  final AdminDashboardCreateUserOperationOutputConfig pageConfig;
  ...
  List<DataColumn> getColumns(Function onAdd, Function onSort) {
    return [
      DataColumn(
        label: Text(
          AppLocalizations.of(context).lookUpValue(context, 'County'),
  ...
  Function getRow({BuildContext context, List<TableRowAction> rowActionList, Function navigateToViewPageAction}) {
    ...
    return (targetCounty) => DataRow(
          cells: [
            pageConfig.activityCountiesTableConfig.representationDataCell != null
                ? pageConfig.activityCountiesTableConfig.representationDataCell(targetCounty, context)
                : DataCell(
                    ...
                    onTap: !disabled
                        ? () => _customOnTapOrNavigateToView(
                            pageConfig.activityCountiesTableConfig.representationDataCellOnTap, navigateToViewPageAction, targetCounty)
                        : null,
                  ),
            DataCell(TableUtility.getRowActionsWidget(pageConfig.activityCountiesTableConfig, rowActionList, targetCounty, pageStore.pageState)),
          ],
        );
  }
