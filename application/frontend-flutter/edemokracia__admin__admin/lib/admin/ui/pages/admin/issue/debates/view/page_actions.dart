...
class AdminIssueDebatesViewPageActions extends PageActions {
  NavigationState navigation;
  final messageHandler = locator<MessageHandler>();

  final AdminIssueDebateStore targetStore;
  final AdminIssueStore ownerStore;
  final AdminIssueDebatesViewPageStore pageStore;
  final AdminIssueDebatesViewConfig pageConfig;

  AdminIssueDebatesViewPageActions(this.navigation, this.targetStore, this.ownerStore, this.pageStore, this.pageConfig);
  ...
  List<Widget> getActions(BuildContext context) {
    ...
    list.add(JudoAppBarButton(
      ...
      onPressed: () async {
        try {
          await pageStore.refreshIssueDebate(targetStore);
        } catch (error) {
