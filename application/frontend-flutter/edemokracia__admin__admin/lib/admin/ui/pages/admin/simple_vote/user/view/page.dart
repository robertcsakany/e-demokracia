...
class AdminSimpleVoteUserViewPageStore extends _AdminSimpleVoteUserViewPageStore with _$AdminSimpleVoteUserViewPageStore {}

abstract class _AdminSimpleVoteUserViewPageStore with Store {
  ...
  _AdminSimpleVoteUserViewPageStore() {
  ...
  void initSortAggregatedLists(BuildContext context, AdminSimpleVoteUserViewConfig pageConfig) {
    if (targetStore.activityCities != null) {
      var dataInfo = EdemokraciaAdminSimpleVoteUserViewActivityCitiesMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.activityDistricts != null) {
      var dataInfo = EdemokraciaAdminSimpleVoteUserViewActivityDistrictsMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.activityCounties != null) {
      var dataInfo = EdemokraciaAdminSimpleVoteUserViewActivityCountiesMobileTable(context, this, pageConfig, disabled: false);
  ...
  void activityCitiesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminSimpleVoteUserViewActivityCities',
  ...
  void activityCountiesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminSimpleVoteUserViewActivityCounties',
  ...
  void activityDistrictsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminSimpleVoteUserViewActivityDistricts',
...
class AdminSimpleVoteUserViewPage extends StatefulWidget {
  static const String title = 'View / Edit User';

  final AdminUserStore targetStore;
  final AdminSimpleVoteStore ownerStore;

  const AdminSimpleVoteUserViewPage({Key key, @required this.targetStore, @required this.ownerStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminSimpleVoteUserViewPageState(targetStore: targetStore, ownerStore: ownerStore);
}

class _AdminSimpleVoteUserViewPageState extends State<AdminSimpleVoteUserViewPage> {
  AdminUserStore targetStore;
  final AdminSimpleVoteStore ownerStore;

  final tableService = locator<TableService>();
  final AdminSimpleVoteUserViewPageStore pageStore = AdminSimpleVoteUserViewPageStore();
  final AdminSimpleVoteUserViewConfig pageConfig = AdminSimpleVoteUserViewConfig();
  AdminSimpleVoteUserViewPageActions pageActions;

  _AdminSimpleVoteUserViewPageState({@required this.targetStore, @required this.ownerStore}) {
    ...
    pageActions = AdminSimpleVoteUserViewPageActions(navigation, targetStore, ownerStore, pageStore, pageConfig);
    pageStore.refreshUser(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminSimpleVoteUserViewPage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminSimpleVoteUserViewMobilePage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminSimpleVoteUserViewTabletPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminSimpleVoteUserViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      return getAdminSimpleVoteUserViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
    });
