...
class AdminCountyCitiesViewPageActions extends PageActions {
  NavigationState navigation;
  final messageHandler = locator<MessageHandler>();

  final AdminCityStore targetStore;
  final AdminCountyStore ownerStore;
  final AdminCountyCitiesViewPageStore pageStore;
  final AdminCountyCitiesViewConfig pageConfig;

  AdminCountyCitiesViewPageActions(this.navigation, this.targetStore, this.ownerStore, this.pageStore, this.pageConfig);
  ...
  List<Widget> getActions(BuildContext context) {
    ...
    list.add(JudoAppBarButton(
      ...
      onPressed: () async {
        try {
          await pageStore.deleteCity(targetStore);
    ...
    list.add(JudoAppBarButton(
      ...
      onPressed: () async {
        ...
        AdminCityStore cloned = AdminCityStore.clone(targetStore);
        var res = await navigation.open(Routes.adminCountyCitiesUpdatePage, arguments: AdminCountyCitiesUpdatePageArguments(targetStore: cloned));
        if (res != null) {
          try {
            await pageStore.updateCity(cloned, targetStore);
          } catch (error) {
