...
class AdminConProsUpdatePageStore extends _AdminConProsUpdatePageStore with _$AdminConProsUpdatePageStore {}

abstract class _AdminConProsUpdatePageStore with Store {
  ...
  void initSortAggregatedLists(BuildContext context, AdminConProsUpdateConfig pageConfig) {
    if (targetStore.cons != null) {
      var dataInfo = EdemokraciaAdminConProsEditConsMobileTable(context, this, pageConfig, disabled: true);
      targetStore.cons.sort(TableUtility.getSortComparator(consSortColumnIndex, consSortAsc, dataInfo));
    }
    if (targetStore.pros != null) {
      var dataInfo = EdemokraciaAdminConProsEditProsMobileTable(context, this, pageConfig, disabled: true);
    ...
    if (targetStore.comments != null) {
      var dataInfo = EdemokraciaAdminConProsEditCommentsMobileTable(context, this, pageConfig, disabled: true);
  ...
  void commentsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminProStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConProsEditComments',
  ...
  void consSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminProStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConProsEditCons',
  ...
  void prosSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminProStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConProsEditPros',
...
class AdminConProsUpdatePage extends StatefulWidget {
  static const String title = 'View / Edit Pro';

  final AdminProStore targetStore;

  const AdminConProsUpdatePage({Key key, @required this.targetStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminConProsUpdatePageState(targetStore: targetStore);
}

class _AdminConProsUpdatePageState extends State<AdminConProsUpdatePage> {
  final AdminProStore targetStore;

  final tableService = locator<TableService>();
  final AdminConProsUpdatePageStore pageStore = AdminConProsUpdatePageStore();
  final AdminConProsUpdateConfig pageConfig = AdminConProsUpdateConfig();
  AdminConProsUpdatePageActions pageActions;

  _AdminConProsUpdatePageState({@required this.targetStore}) {
    ...
    pageActions = AdminConProsUpdatePageActions(navigation, targetStore, pageStore, pageConfig);
    pageStore.refreshPro(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminConProsUpdatePage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminConProsUpdateMobilePage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminConProsUpdateTabletPage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminConProsUpdateDesktopPage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      return getAdminConProsUpdateDesktopPage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
    });
