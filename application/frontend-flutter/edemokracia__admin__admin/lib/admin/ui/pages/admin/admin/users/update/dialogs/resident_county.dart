...
// Table: residentCounty

part of admin.ui.pages.admin.admin.users.update;

class AdminAdminUsersUpdateResidentCountyDialogStore extends _AdminAdminUsersUpdateResidentCountyDialogStore
    with _$AdminAdminUsersUpdateResidentCountyDialogStore {}

abstract class _AdminAdminUsersUpdateResidentCountyDialogStore with Store {
...
Future<AdminCountyStore> judoSelectAdminAdminUsersUpdateResidentCountyDialog(
  ...
  var dialogHelperStore = AdminAdminUsersUpdateResidentCountyDialogStore();
  dialogHelperStore.numberOfColumns = 1;
  dialogHelperStore.numberOfColumnsAfterFilterHorizontal = pageConfig.residentCountyTableConfig.numberOfColumnsAfterFilterHorizontalInDialog;
  if (pageConfig.residentCountyTableConfig.defaultOpenedFilters.isNotEmpty) {
    dialogHelperStore.availableFilterList.addAll(pageConfig.residentCountyTableConfig.defaultOpenedFilters);
    setFiltersLocalizedLabel(context, dialogHelperStore.availableFilterList);
  }
  if (pageConfig.residentCountyTableConfig.selectableFilters.isNotEmpty) {
    dialogHelperStore.selectableFilters.clear();
    dialogHelperStore.selectableFilters.addAll(pageConfig.residentCountyTableConfig.selectableFilters);
  }

  try {
    await pageStore.getRangeForResidentCounty(
      ownerStore,
  ...
  ObservableList<AdminCountyStore> selected = <AdminCountyStore>[].asObservable();

  var tableHelperStore = TableHelperStore(selected, (AdminCountyStore value) => value?.internal__identifier);
  if (ownerStore.residentCounty != null) {
    tableHelperStore.singleSelected = ownerStore.residentCounty;
  }

  return await showDialog<AdminCountyStore>(
    context: context,
    builder: (context) {
      return AlertDialog(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Observer(builder: (_) {
                      switch (pageStore.userResidentCountyStoreRangeFuture.status) {
                        case FutureStatus.pending:
                          return JudoLoadingProgress();
                        case FutureStatus.rejected:
                          return Container();
                        case FutureStatus.fulfilled:
                          var dataInfo = AdminAdminUsersUpdateResidentCountyDialogTable(context, pageStore, pageConfig, disabled: false);
                          return JudoSelectorTable(
                            ...
                            row: 1.0 - 1 >= 6 ? 1.0 - 1 : 6,
                            dataInfo: dataInfo,
                            sortAscending: pageStore.residentCountySortAscRange,
                            sortColumnIndex: pageStore.residentCountySortColumnIndexRange,
                            onSort: (int columnIndex, bool asc) async {
                              try {
                                await pageStore.residentCountySetSortRange(
                                  ownerStore,
                                  context,
                            ...
                            rowList: pageStore.residentCountyStoreRangeList,
                  ...
                  child: Row(
                    children: [
                      Padding(
                        ...
                        child: Observer(
                          builder: (_) => Text(
                            pageStore.residentCountyTableForRangeItemsRangeEnd != 0
                                ? '${pageStore.residentCountyTableForRangeItemsRangeStart} - ${pageStore.residentCountyTableForRangeItemsRangeEnd}'
                      ...
                      Observer(
                        builder: (_) => Padding(
                          ...
                          child: ElevatedButton(
                            onPressed: pageStore.previousButtonEnableForResidentCountyRange
                                ? () async {
                                    try {
                                      await pageStore.getRangeForResidentCounty(
                                        ownerStore,
                      ...
                      Observer(
                        builder: (_) => Padding(
                          padding: JudoComponentCustomizer.get().getDefaultPadding(),
                          child: ElevatedButton(
                            onPressed: pageStore.nextButtonEnableForResidentCountyRange
                                ? () async {
                                    try {
                                      await pageStore.getRangeForResidentCounty(
                                        ownerStore,
