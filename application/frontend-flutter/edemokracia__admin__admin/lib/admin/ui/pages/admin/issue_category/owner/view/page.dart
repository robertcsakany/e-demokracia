...
class AdminIssueCategoryOwnerViewPageStore extends _AdminIssueCategoryOwnerViewPageStore with _$AdminIssueCategoryOwnerViewPageStore {}

abstract class _AdminIssueCategoryOwnerViewPageStore with Store {
  ...
  _AdminIssueCategoryOwnerViewPageStore() {
  ...
  void initSortAggregatedLists(BuildContext context, AdminIssueCategoryOwnerViewConfig pageConfig) {
    if (targetStore.activityCities != null) {
      var dataInfo = EdemokraciaAdminIssueCategoryOwnerViewActivityCitiesMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.activityDistricts != null) {
      var dataInfo = EdemokraciaAdminIssueCategoryOwnerViewActivityDistrictsMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.activityCounties != null) {
      var dataInfo = EdemokraciaAdminIssueCategoryOwnerViewActivityCountiesMobileTable(context, this, pageConfig, disabled: false);
  ...
  void activityCitiesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminIssueCategoryOwnerViewActivityCities',
  ...
  void activityCountiesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminIssueCategoryOwnerViewActivityCounties',
  ...
  void activityDistrictsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminIssueCategoryOwnerViewActivityDistricts',
...
class AdminIssueCategoryOwnerViewPage extends StatefulWidget {
  static const String title = 'View / Edit User';

  final AdminUserStore targetStore;
  final AdminIssueCategoryStore ownerStore;

  const AdminIssueCategoryOwnerViewPage({Key key, @required this.targetStore, @required this.ownerStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminIssueCategoryOwnerViewPageState(targetStore: targetStore, ownerStore: ownerStore);
}

class _AdminIssueCategoryOwnerViewPageState extends State<AdminIssueCategoryOwnerViewPage> {
  AdminUserStore targetStore;
  final AdminIssueCategoryStore ownerStore;

  final tableService = locator<TableService>();
  final AdminIssueCategoryOwnerViewPageStore pageStore = AdminIssueCategoryOwnerViewPageStore();
  final AdminIssueCategoryOwnerViewConfig pageConfig = AdminIssueCategoryOwnerViewConfig();
  AdminIssueCategoryOwnerViewPageActions pageActions;

  _AdminIssueCategoryOwnerViewPageState({@required this.targetStore, @required this.ownerStore}) {
    ...
    pageActions = AdminIssueCategoryOwnerViewPageActions(navigation, targetStore, ownerStore, pageStore, pageConfig);
    pageStore.refreshUser(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminIssueCategoryOwnerViewPage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminIssueCategoryOwnerViewMobilePage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminIssueCategoryOwnerViewTabletPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminIssueCategoryOwnerViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      return getAdminIssueCategoryOwnerViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
    });
