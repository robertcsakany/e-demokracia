...
class AdminIssueCategorySubcategoriesCreatePageStore extends _AdminIssueCategorySubcategoriesCreatePageStore
    with _$AdminIssueCategorySubcategoriesCreatePageStore {}

abstract class _AdminIssueCategorySubcategoriesCreatePageStore with Store {
  ...
  _AdminIssueCategorySubcategoriesCreatePageStore() {
  ...
  @action
  Future<AdminIssueCategoryStore> validate(AdminIssueCategoryStore ownerStore, AdminIssueCategoryStore targetStore) async {
    return await _actorRepository.edemokraciaAdminIssueCategorySubcategoriesValidateForCreate(ownerStore, targetStore);
  }
  ...
  void ownerSetSortRange(AdminIssueCategoryStore ownerStore, BuildContext context, String sortColumnName, int sortColumnIndex, bool asc,
    ...
    tableService.storeSorting(
      'EdemokraciaAdminIssueCategorySubcategoriesCreateOwnerRange',
  ...
  void subcategoriesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminIssueCategoryStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminIssueCategorySubcategoriesCreateSubcategories',
...
class AdminIssueCategorySubcategoriesCreatePage extends StatefulWidget {
  static const String title = 'Create Category';

  final AdminIssueCategoryStore ownerStore;

  const AdminIssueCategorySubcategoriesCreatePage({Key key, @required this.ownerStore}) : super(key: key);

  @override
  _AdminIssueCategorySubcategoriesCreatePageState createState() => _AdminIssueCategorySubcategoriesCreatePageState(ownerStore: ownerStore);
}

class _AdminIssueCategorySubcategoriesCreatePageState extends State<AdminIssueCategorySubcategoriesCreatePage> {
  final AdminIssueCategoryStore targetStore = AdminIssueCategoryStore();
  final AdminIssueCategoryStore ownerStore;

  final tableService = locator<TableService>();
  final AdminIssueCategorySubcategoriesCreatePageStore pageStore = AdminIssueCategorySubcategoriesCreatePageStore();
  final AdminIssueCategorySubcategoriesCreateConfig pageConfig = AdminIssueCategorySubcategoriesCreateConfig();
  AdminIssueCategorySubcategoriesCreatePageActions pageActions;
  ...
  _AdminIssueCategorySubcategoriesCreatePageState({@required this.ownerStore}) {
    ...
    pageActions = AdminIssueCategorySubcategoriesCreatePageActions(navigation, targetStore, ownerStore, pageStore, pageConfig);
    SchedulerBinding.instance.addPostFrameCallback((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminIssueCategorySubcategoriesCreatePage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminIssueCategorySubcategoriesCreateMobilePage(context, targetStore, ownerStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminIssueCategorySubcategoriesCreateTabletPage(context, targetStore, ownerStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminIssueCategorySubcategoriesCreateDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      return getAdminIssueCategorySubcategoriesCreateDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig, inputWidgetKeyMap);
    });
