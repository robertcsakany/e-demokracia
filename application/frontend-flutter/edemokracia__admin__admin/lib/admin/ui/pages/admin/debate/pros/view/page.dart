...
class AdminDebateProsViewPageStore extends _AdminDebateProsViewPageStore with _$AdminDebateProsViewPageStore {}

abstract class _AdminDebateProsViewPageStore with Store {
  ...
  void initSortAggregatedLists(BuildContext context, AdminDebateProsViewConfig pageConfig) {
    if (targetStore.cons != null) {
      var dataInfo = EdemokraciaAdminDebateProsViewConsMobileTable(context, this, pageConfig, disabled: false);
      targetStore.cons.sort(TableUtility.getSortComparator(consSortColumnIndex, consSortAsc, dataInfo));
    }
    if (targetStore.pros != null) {
      var dataInfo = EdemokraciaAdminDebateProsViewProsMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.comments != null) {
      var dataInfo = EdemokraciaAdminDebateProsViewCommentsMobileTable(context, this, pageConfig, disabled: false);
  ...
  void commentsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminProStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminDebateProsViewComments',
  ...
  void consSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminProStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminDebateProsViewCons',
  ...
  void prosSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminProStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminDebateProsViewPros',
...
class AdminDebateProsViewPage extends StatefulWidget {
  static const String title = 'View / Edit Pro';

  final AdminProStore targetStore;
  final AdminDebateStore ownerStore;

  const AdminDebateProsViewPage({Key key, @required this.targetStore, @required this.ownerStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminDebateProsViewPageState(targetStore: targetStore, ownerStore: ownerStore);
}

class _AdminDebateProsViewPageState extends State<AdminDebateProsViewPage> {
  AdminProStore targetStore;
  final AdminDebateStore ownerStore;

  final tableService = locator<TableService>();
  final AdminDebateProsViewPageStore pageStore = AdminDebateProsViewPageStore();
  final AdminDebateProsViewConfig pageConfig = AdminDebateProsViewConfig();
  AdminDebateProsViewPageActions pageActions;

  _AdminDebateProsViewPageState({@required this.targetStore, @required this.ownerStore}) {
    ...
    pageActions = AdminDebateProsViewPageActions(navigation, targetStore, ownerStore, pageStore, pageConfig);
    pageStore.refreshPro(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminDebateProsViewPage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminDebateProsViewMobilePage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminDebateProsViewTabletPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminDebateProsViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      return getAdminDebateProsViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
    });
