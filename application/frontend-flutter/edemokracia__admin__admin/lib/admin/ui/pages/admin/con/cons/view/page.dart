...
class AdminConConsViewPageStore extends _AdminConConsViewPageStore with _$AdminConConsViewPageStore {}

abstract class _AdminConConsViewPageStore with Store {
  ...
  void initSortAggregatedLists(BuildContext context, AdminConConsViewConfig pageConfig) {
    if (targetStore.cons != null) {
      var dataInfo = EdemokraciaAdminConConsViewConsMobileTable(context, this, pageConfig, disabled: false);
      targetStore.cons.sort(TableUtility.getSortComparator(consSortColumnIndex, consSortAsc, dataInfo));
    }
    if (targetStore.pros != null) {
      var dataInfo = EdemokraciaAdminConConsViewProsMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.comments != null) {
      var dataInfo = EdemokraciaAdminConConsViewCommentsMobileTable(context, this, pageConfig, disabled: false);
  ...
  void commentsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminConStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConConsViewComments',
  ...
  void consSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminConStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConConsViewCons',
  ...
  void prosSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminConStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConConsViewPros',
...
class AdminConConsViewPage extends StatefulWidget {
  static const String title = 'View / Edit Con';

  final AdminConStore targetStore;
  final AdminConStore ownerStore;

  const AdminConConsViewPage({Key key, @required this.targetStore, @required this.ownerStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminConConsViewPageState(targetStore: targetStore, ownerStore: ownerStore);
}

class _AdminConConsViewPageState extends State<AdminConConsViewPage> {
  AdminConStore targetStore;
  final AdminConStore ownerStore;

  final tableService = locator<TableService>();
  final AdminConConsViewPageStore pageStore = AdminConConsViewPageStore();
  final AdminConConsViewConfig pageConfig = AdminConConsViewConfig();
  AdminConConsViewPageActions pageActions;

  _AdminConConsViewPageState({@required this.targetStore, @required this.ownerStore}) {
    ...
    pageActions = AdminConConsViewPageActions(navigation, targetStore, ownerStore, pageStore, pageConfig);
    pageStore.refreshCon(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminConConsViewPage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminConConsViewMobilePage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminConConsViewTabletPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminConConsViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      return getAdminConConsViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
    });
