...
class AdminAdminUsersViewPageStore extends _AdminAdminUsersViewPageStore with _$AdminAdminUsersViewPageStore {}

abstract class _AdminAdminUsersViewPageStore with Store {
  ...
  void initSortAggregatedLists(BuildContext context, AdminAdminUsersViewConfig pageConfig) {
    if (targetStore.activityCities != null) {
      var dataInfo = EdemokraciaAdminAdminUsersViewActivityCitiesMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.activityDistricts != null) {
      var dataInfo = EdemokraciaAdminAdminUsersViewActivityDistrictsMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.activityCounties != null) {
      var dataInfo = EdemokraciaAdminAdminUsersViewActivityCountiesMobileTable(context, this, pageConfig, disabled: false);
  ...
  @action
  Future<void> updateUser(AdminUserStore newTargetStore, AdminUserStore oldTargetStore) async {
    await _actorRepository.edemokraciaAdminUserUpdate(newTargetStore);
    await refreshUser(oldTargetStore);
  }
  ...
  void activityCitiesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminAdminUsersViewActivityCities',
  ...
  void activityCountiesSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminAdminUsersViewActivityCounties',
  ...
  void activityDistrictsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminUserStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminAdminUsersViewActivityDistricts',
...
class AdminAdminUsersViewPage extends StatefulWidget {
  static const String title = 'View / Edit User';

  final String idParam;

  const AdminAdminUsersViewPage({Key key, @PathParam('id') this.idParam}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminAdminUsersViewPageState(idParam: idParam);
}

class _AdminAdminUsersViewPageState extends State<AdminAdminUsersViewPage> {
  final String idParam;
  AdminUserStore targetStore;

  final tableService = locator<TableService>();
  final AdminAdminUsersViewPageStore pageStore = AdminAdminUsersViewPageStore();
  final AdminAdminUsersViewConfig pageConfig = AdminAdminUsersViewConfig();
  AdminAdminUsersViewPageActions pageActions;

  _AdminAdminUsersViewPageState({this.idParam}) {
    ...
    targetStore = AdminUserStore();
    targetStore.internal__signedIdentifier = idParam;
    pageStore.targetStore = targetStore;
    pageActions = AdminAdminUsersViewPageActions(navigation, targetStore, pageStore, pageConfig);
    pageStore.refreshUser(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminAdminUsersViewPage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminAdminUsersViewMobilePage(context, targetStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminAdminUsersViewTabletPage(context, targetStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminAdminUsersViewDesktopPage(context, targetStore, pageStore, pageConfig);
      }
      return getAdminAdminUsersViewDesktopPage(context, targetStore, pageStore, pageConfig);
    });
