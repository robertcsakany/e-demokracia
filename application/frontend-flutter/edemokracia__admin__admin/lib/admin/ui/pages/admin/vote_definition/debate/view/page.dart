...
class AdminVoteDefinitionDebateViewPageStore extends _AdminVoteDefinitionDebateViewPageStore with _$AdminVoteDefinitionDebateViewPageStore {}

abstract class _AdminVoteDefinitionDebateViewPageStore with Store {
  ...
  _AdminVoteDefinitionDebateViewPageStore() {
  ...
  void initSortAggregatedLists(BuildContext context, AdminVoteDefinitionDebateViewConfig pageConfig) {
    if (targetStore.cons != null) {
      var dataInfo = EdemokraciaAdminVoteDefinitionDebateViewConsMobileTable(context, this, pageConfig, disabled: false);
      targetStore.cons.sort(TableUtility.getSortComparator(consSortColumnIndex, consSortAsc, dataInfo));
    }
    if (targetStore.pros != null) {
      var dataInfo = EdemokraciaAdminVoteDefinitionDebateViewProsMobileTable(context, this, pageConfig, disabled: false);
    ...
    if (targetStore.comments != null) {
      var dataInfo = EdemokraciaAdminVoteDefinitionDebateViewCommentsMobileTable(context, this, pageConfig, disabled: false);
  ...
  void commentsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminDebateStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminVoteDefinitionDebateViewComments',
  ...
  void consSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminDebateStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminVoteDefinitionDebateViewCons',
  ...
  void prosSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminDebateStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminVoteDefinitionDebateViewPros',
...
class AdminVoteDefinitionDebateViewPage extends StatefulWidget {
  static const String title = 'View / Edit Debate';

  final AdminDebateStore targetStore;
  final AdminVoteDefinitionStore ownerStore;

  const AdminVoteDefinitionDebateViewPage({Key key, @required this.targetStore, @required this.ownerStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminVoteDefinitionDebateViewPageState(targetStore: targetStore, ownerStore: ownerStore);
}

class _AdminVoteDefinitionDebateViewPageState extends State<AdminVoteDefinitionDebateViewPage> {
  AdminDebateStore targetStore;
  final AdminVoteDefinitionStore ownerStore;

  final tableService = locator<TableService>();
  final AdminVoteDefinitionDebateViewPageStore pageStore = AdminVoteDefinitionDebateViewPageStore();
  final AdminVoteDefinitionDebateViewConfig pageConfig = AdminVoteDefinitionDebateViewConfig();
  AdminVoteDefinitionDebateViewPageActions pageActions;

  _AdminVoteDefinitionDebateViewPageState({@required this.targetStore, @required this.ownerStore}) {
    ...
    pageActions = AdminVoteDefinitionDebateViewPageActions(navigation, targetStore, ownerStore, pageStore, pageConfig);
    pageStore.refreshDebate(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminVoteDefinitionDebateViewPage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminVoteDefinitionDebateViewMobilePage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminVoteDefinitionDebateViewTabletPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminVoteDefinitionDebateViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      return getAdminVoteDefinitionDebateViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
    });
