...
class AdminCityDistrictsViewPageActions extends PageActions {
  NavigationState navigation;
  final messageHandler = locator<MessageHandler>();

  final AdminDistrictStore targetStore;
  final AdminCityStore ownerStore;
  final AdminCityDistrictsViewPageStore pageStore;
  final AdminCityDistrictsViewConfig pageConfig;

  AdminCityDistrictsViewPageActions(this.navigation, this.targetStore, this.ownerStore, this.pageStore, this.pageConfig);
  ...
  List<Widget> getActions(BuildContext context) {
    ...
    list.add(JudoAppBarButton(
      ...
      onPressed: () async {
        try {
          await pageStore.refreshDistrict(targetStore);
        } catch (error) {
    ...
    list.add(JudoAppBarButton(
      ...
      onPressed: () async {
        try {
          await pageStore.deleteDistrict(targetStore);
    ...
    list.add(JudoAppBarButton(
      ...
      onPressed: () async {
        ...
        AdminDistrictStore cloned = AdminDistrictStore.clone(targetStore);
        var res = await navigation.open(Routes.adminCityDistrictsUpdatePage, arguments: AdminCityDistrictsUpdatePageArguments(targetStore: cloned));
        if (res != null) {
          try {
            await pageStore.updateDistrict(cloned, targetStore);
          } catch (error) {
