...
class AdminConConsUpdatePageStore extends _AdminConConsUpdatePageStore with _$AdminConConsUpdatePageStore {}

abstract class _AdminConConsUpdatePageStore with Store {
  ...
  void initSortAggregatedLists(BuildContext context, AdminConConsUpdateConfig pageConfig) {
    if (targetStore.cons != null) {
      var dataInfo = EdemokraciaAdminConConsEditConsMobileTable(context, this, pageConfig, disabled: true);
      targetStore.cons.sort(TableUtility.getSortComparator(consSortColumnIndex, consSortAsc, dataInfo));
    }
    if (targetStore.pros != null) {
      var dataInfo = EdemokraciaAdminConConsEditProsMobileTable(context, this, pageConfig, disabled: true);
    ...
    if (targetStore.comments != null) {
      var dataInfo = EdemokraciaAdminConConsEditCommentsMobileTable(context, this, pageConfig, disabled: true);
  ...
  void commentsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminConStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConConsEditComments',
  ...
  void consSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminConStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConConsEditCons',
  ...
  void prosSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminConStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminConConsEditPros',
...
class AdminConConsUpdatePage extends StatefulWidget {
  static const String title = 'View / Edit Con';

  final AdminConStore targetStore;

  const AdminConConsUpdatePage({Key key, @required this.targetStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminConConsUpdatePageState(targetStore: targetStore);
}

class _AdminConConsUpdatePageState extends State<AdminConConsUpdatePage> {
  final AdminConStore targetStore;

  final tableService = locator<TableService>();
  final AdminConConsUpdatePageStore pageStore = AdminConConsUpdatePageStore();
  final AdminConConsUpdateConfig pageConfig = AdminConConsUpdateConfig();
  AdminConConsUpdatePageActions pageActions;

  _AdminConConsUpdatePageState({@required this.targetStore}) {
    ...
    pageActions = AdminConConsUpdatePageActions(navigation, targetStore, pageStore, pageConfig);
    pageStore.refreshCon(targetStore).then((_) {
      navigation.setCurrentTitle(pageConfig.titleGenerator != null
          ? pageConfig.titleGenerator(context, pageStore, targetStore)
          : AppLocalizations.of(context).lookUpValue(context, AdminConConsUpdatePage.title));
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminConConsUpdateMobilePage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminConConsUpdateTabletPage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminConConsUpdateDesktopPage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
      }
      return getAdminConConsUpdateDesktopPage(context, targetStore, pageStore, pageConfig, inputWidgetKeyMap);
    });
