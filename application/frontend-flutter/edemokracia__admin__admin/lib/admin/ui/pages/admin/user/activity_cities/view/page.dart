...
class AdminUserActivityCitiesViewPageStore extends _AdminUserActivityCitiesViewPageStore with _$AdminUserActivityCitiesViewPageStore {}

abstract class _AdminUserActivityCitiesViewPageStore with Store {
  ...
  _AdminUserActivityCitiesViewPageStore() {
  ...
  void initSortAggregatedLists(BuildContext context, AdminUserActivityCitiesViewConfig pageConfig) {
    if (targetStore.districts != null) {
      var dataInfo = EdemokraciaAdminUserActivityCitiesViewDistrictsMobileTable(context, this, pageConfig, disabled: false);
  ...
  void districtsSetSort(BuildContext context, String sortColumnName, int sortColumnIndex, bool asc, Comparator sortCompare, AdminCityStore store) {
    ...
    tableService.storeSorting(
      'EdemokraciaAdminUserActivityCitiesViewDistricts',
...
class AdminUserActivityCitiesViewPage extends StatefulWidget {
  static const String title = 'City';

  final AdminCityStore targetStore;
  final AdminUserStore ownerStore;

  const AdminUserActivityCitiesViewPage({Key key, @required this.targetStore, @required this.ownerStore}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _AdminUserActivityCitiesViewPageState(targetStore: targetStore, ownerStore: ownerStore);
}

class _AdminUserActivityCitiesViewPageState extends State<AdminUserActivityCitiesViewPage> {
  AdminCityStore targetStore;
  final AdminUserStore ownerStore;

  final tableService = locator<TableService>();
  final AdminUserActivityCitiesViewPageStore pageStore = AdminUserActivityCitiesViewPageStore();
  final AdminUserActivityCitiesViewConfig pageConfig = AdminUserActivityCitiesViewConfig();
  AdminUserActivityCitiesViewPageActions pageActions;

  _AdminUserActivityCitiesViewPageState({@required this.targetStore, @required this.ownerStore}) {
    ...
    pageActions = AdminUserActivityCitiesViewPageActions(navigation, targetStore, ownerStore, pageStore, pageConfig);
  ...
  Widget build(BuildContext context) {
    LayoutBuilder layout = LayoutBuilder(builder: (context, constraints) {
      if (MediaQuery.of(context).size.width >= 0 && MediaQuery.of(context).size.width <= 599) {
        return getAdminUserActivityCitiesViewMobilePage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 600 && MediaQuery.of(context).size.width <= 839) {
        return getAdminUserActivityCitiesViewTabletPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      if (MediaQuery.of(context).size.width >= 840 && MediaQuery.of(context).size.width <= 32767) {
        return getAdminUserActivityCitiesViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
      }
      return getAdminUserActivityCitiesViewDesktopPage(context, targetStore, ownerStore, pageStore, pageConfig);
    });
