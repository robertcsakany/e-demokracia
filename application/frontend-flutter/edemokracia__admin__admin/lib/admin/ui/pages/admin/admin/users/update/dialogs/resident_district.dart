...
// Table: residentDistrict

part of admin.ui.pages.admin.admin.users.update;

class AdminAdminUsersUpdateResidentDistrictDialogStore extends _AdminAdminUsersUpdateResidentDistrictDialogStore
    with _$AdminAdminUsersUpdateResidentDistrictDialogStore {}

abstract class _AdminAdminUsersUpdateResidentDistrictDialogStore with Store {
...
Future<AdminDistrictStore> judoSelectAdminAdminUsersUpdateResidentDistrictDialog(
  ...
  var dialogHelperStore = AdminAdminUsersUpdateResidentDistrictDialogStore();
  dialogHelperStore.numberOfColumns = 1;
  dialogHelperStore.numberOfColumnsAfterFilterHorizontal = pageConfig.residentDistrictTableConfig.numberOfColumnsAfterFilterHorizontalInDialog;
  if (pageConfig.residentDistrictTableConfig.defaultOpenedFilters.isNotEmpty) {
    dialogHelperStore.availableFilterList.addAll(pageConfig.residentDistrictTableConfig.defaultOpenedFilters);
    setFiltersLocalizedLabel(context, dialogHelperStore.availableFilterList);
  }
  if (pageConfig.residentDistrictTableConfig.selectableFilters.isNotEmpty) {
    dialogHelperStore.selectableFilters.clear();
    dialogHelperStore.selectableFilters.addAll(pageConfig.residentDistrictTableConfig.selectableFilters);
  }

  try {
    await pageStore.getRangeForResidentDistrict(
      ownerStore,
  ...
  ObservableList<AdminDistrictStore> selected = <AdminDistrictStore>[].asObservable();

  var tableHelperStore = TableHelperStore(selected, (AdminDistrictStore value) => value?.internal__identifier);
  if (ownerStore.residentDistrict != null) {
    tableHelperStore.singleSelected = ownerStore.residentDistrict;
  }

  return await showDialog<AdminDistrictStore>(
    context: context,
    builder: (context) {
      return AlertDialog(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Observer(builder: (_) {
                      switch (pageStore.userResidentDistrictStoreRangeFuture.status) {
                        case FutureStatus.pending:
                          return JudoLoadingProgress();
                        case FutureStatus.rejected:
                          return Container();
                        case FutureStatus.fulfilled:
                          var dataInfo = AdminAdminUsersUpdateResidentDistrictDialogTable(context, pageStore, pageConfig, disabled: false);
                          return JudoSelectorTable(
                            ...
                            row: 1.0 - 1 >= 6 ? 1.0 - 1 : 6,
                            dataInfo: dataInfo,
                            sortAscending: pageStore.residentDistrictSortAscRange,
                            sortColumnIndex: pageStore.residentDistrictSortColumnIndexRange,
                            onSort: (int columnIndex, bool asc) async {
                              try {
                                await pageStore.residentDistrictSetSortRange(
                                  ownerStore,
                                  context,
                            ...
                            rowList: pageStore.residentDistrictStoreRangeList,
                  ...
                  child: Row(
                    children: [
                      Padding(
                        ...
                        child: Observer(
                          builder: (_) => Text(
                            pageStore.residentDistrictTableForRangeItemsRangeEnd != 0
                                ? '${pageStore.residentDistrictTableForRangeItemsRangeStart} - ${pageStore.residentDistrictTableForRangeItemsRangeEnd}'
                      ...
                      Observer(
                        builder: (_) => Padding(
                          ...
                          child: ElevatedButton(
                            onPressed: pageStore.previousButtonEnableForResidentDistrictRange
                                ? () async {
                                    try {
                                      await pageStore.getRangeForResidentDistrict(
                                        ownerStore,
                      ...
                      Observer(
                        builder: (_) => Padding(
                          padding: JudoComponentCustomizer.get().getDefaultPadding(),
                          child: ElevatedButton(
                            onPressed: pageStore.nextButtonEnableForResidentDistrictRange
                                ? () async {
                                    try {
                                      await pageStore.getRangeForResidentDistrict(
                                        ownerStore,
