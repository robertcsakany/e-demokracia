...
class AdminUserVotesTablePageActions extends PageActions {
  ...
  AdminUserVotesTablePageActions(this.navigation, this.ownerStore, this.pageStore, this.pageConfig);
  ...
  List<Widget> getActions(BuildContext context) {
      ...
      onPressed: () async {
        showDialog(
            ...
            builder: (_) => new AlertDialog(
                  ...
                  content: new Text(
                    AppLocalizations.of(context).lookUpValue(context, 'Are you sure you would like to clear all elements in the list?'),
                  ),
                  actions: <Widget>[
                    new TextButton(
                      onPressed: () {
                    ...
                    new ElevatedButton(
                      onPressed: () async {
                        try {
                          await pageStore.setVotes(ownerStore, []);
                        } catch (error) {
    ...
    list.add(JudoAppBarButton(
      ...
      padding: EdgeInsets.symmetric(horizontal: 8.0),
      onPressed: () async {
        var selected = await judoSelectAdminUserVotesTableDialog(
          context: context,
        ...
        if (selected != null) {
          try {
            await pageStore.addVotes(ownerStore, selected);
          } catch (error) {
