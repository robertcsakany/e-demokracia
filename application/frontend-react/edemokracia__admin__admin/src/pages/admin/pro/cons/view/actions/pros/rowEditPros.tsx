///////////////////////////////////////////////////////////////////////////////
// G E N E R A T E D    S O U R C E
// ------------------------------
// Factory expression: #getActionsForPages(#application)
// Path expression: #pagePath(#self.value)+'actions/'+#pageActionPathSuffix(#self.key,#self.value)+'.tsx'
// Template name: actor/src/pages/actions/action.tsx.hbs
// Action name: edemokracia::admin::Admin::edemokracia::admin::Con::pros#RowEdit
// Action owner name: edemokracia::admin::Pro.cons#View
// Action DataElement name: pros
// Action DataElement owner name: edemokracia::admin::Con
// Action DataElement target name: edemokracia::admin::Pro
// Owner Page name: edemokracia::admin::Pro.cons#View
export const useRowEditProsAction = () => {
  return async function rowEditProsAction() {
    alert('edemokracia::admin::Admin::edemokracia::admin::Con::pros#RowEdit not supported yet!');
  };
};
